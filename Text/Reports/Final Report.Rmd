...
Our original dataset was extracted from Stats NZ. @Data  
...
*Chlorophyll-A* is an organic molecule found in plant cells that allows plants to photosynthesize. The variable Chlorophyll-A is a measure of the concentration of phytoplankton biomass in milligrams per cubic metre. High concentrations of chlorophyll is a symptom of degraded water quality @Chloro2. The national bottom line for this measure is 12. 

*Total Phosphorus* is the sum of all phosphorus forms in the water, including phosphorus bound to sediment. Large amounts of phosphorus in lakes can reduce dissolved oxygen in the water. This can cause low oxygen areas in the lake, where some aquatic life cannot survive @Phos. Total Phosphorus is measured in milligrams per cubic metre and has a national bottom line of 50mg/m3.  

*Total Nitrogen* is the sum of all nitrogens found in the water, including organic nitrogen from plant tissue. An excess of nitrogen in lakes can cause an increase in algae and plant growth, possibly depriving the lake of oxygen @Nitrogen. Total Nitrogen is measured in milligrams per cubic metre and the national bottom line for stratified lakes is 750mg/m3, and for polymictic lakes is 800mg/m3.  
...
Table \ref{tab:laketable} shows the summary statistics for each of the Lake Health variables.   
...
We constructed a correlation matrix for the Lake Dimension variables, as shown in table \ref{tab:pdacorr}. A visualization of this matrix is shown in figure \ref{fig:VisCorr1}. As to be expected, the three Lake Dimension variables have positive correlations with one another. The correlation between lake area and lake depth is moderate, which was weaker than we expected. Lake perimeter has a strong positive correlation with both area and depth. 
  
```{r pdacorr, echo=FALSE}
...
tab <- round(cor(LakeNum), 3)
tab <- as.table(tab)
rownames(tab) <- c("Perimeter", "Depth", "Area")
colnames(tab) <- c("Perimeter", "Depth", "Area")
knitr::kable(tab, caption = "Correlation Matrix")
```
...
We then examined the means and medians of the Lake Dimension variables for the Regions. Here we have produced barplots for the mean and median of depth, shown in figures \ref{fig:MeanDepth} and \ref{fig:MedianDepth}, respectively. What we can tell from the bargraphs that there is not much difference in distribution between the regions in Lake Depth, however, the mean for each region is approximately 5 meters deeper than the median. This suggests that across most regions there are some much deeper lakes. There are some exceptions to this such as Gisborne. Each region has similar mean and median lake depth, area and perimeter, with the exception of Bay of Plenty, which tended to have deeper lakes than other regions.  
...
The eigenvectors are below, and shown visually in figure \ref{fig:egraph}. We can see the first principal component is approximately the average of the Lake Health variables subtract the average of Clarity. Clarity is best represented in the third principal component. If we were to move forward with the principal components we would just retain principal components 1, 2 and 3 as they capture 92.4% of the variance.  
...
```{r egraph, fig.cap = "Visualisation of the Eigenvctors", fig.pos = "center", echo = FALSE}
...
We produced a partition plot to examine the separation which can be seen in figure \ref{fig:partition1}. 
  
  
```{r partition1, fig.cap ="Partition of the all of the Dominant Landcover variables", echo = FALSE}
...
```
  ...
  In the second LDA both groups had prior probabilities of 0.5 (this is because the train happened to split them with an equal number of observations of 1242 each. The partition can be seen in figure \ref{fig:partition2}  

```{r partition2, fig.cap ="Partition of the Native and Pastoral Dominant Landcover variables", echo = FALSE}
lake4$dominant_landcover<- as.factor(lake4$dominant_landcover)
...
The problems we encountered were to do with missing entries and non-normality. When we first looked at our data and the Stats NZ dataset, we noticed there were 12 observations without an entry for Dominant Landcover. The description of the Stats NZ dataset indicated these should be labelled as 'Other' so we analysed them as being in this category. When analysing region, we noticed there was one observation with no entry for region. As there was no indication as to which region this belonged to, we removed it when analysing Region with other variables.  

When analysing the Lake Health variables, we wanted to use ANOVA testing to test for a difference in means between landcovers. This test require normality, and once we tested for normality using an Anderson-Darling test, we found that these variables were not normally distributed. Therefore, we could not use ANOVA, so we used a Kruskal-Wallis test for multivariate equality in means, and then pairwise Wilcox tests to test for differences between pairs of landcovers for each Lake Health variable.  

When we analysed the depth, we found there were many lakes that were very shallow, and thought about how this could influence our other variables. We thought about Clarity, the maximum depth a Secchi disk is visible, and how this could be affected by the depth of the lake, as very shallow lakes may not be deep enough to properly measure their clarity.  

When we did a Principal Component Analysis on the Lake Health variables, we found that the first three principal components captured 92.4% of the variance, so if we were to move forward with a reduced dataset we would use the first three principal components. 

Our Factor Analysis concluded that two factors was insufficient to capture the full dimensionality of the data. We would like to look into th further, and possibly find other variables that we did not include in our investigation.  

Our Linear Discriminant Analysis was only 76.7% accurate. If we were to continue with our analysis we would like to do a Linear Discriminant Analysis to determine whether the Lake Health variables can accurately predict which island the lakes are located in.  
  
We discovered that, for all Lake Health variables, the lakes with predominantly Native landcover had different means to the other types of landcover. Our graphs suggested these lakes were healthier and clearer than lakes in other landcovers. This could be useful to the Department of Conservation, and could aid in efforts to improve the health of our lakes, possibly with the planting of native plants around lakes. Of course, our study cannot determine whether the native landcover had a causal relationship with the health of the lakes, and cannot determine whether the health of a lake could be improved by increasing the amount of native landcover around our lakes, but this is a start. In future, we would like to investigate this further. 

One of our big leaps forward with processing our data was our crude method for identifying which regions tended to have poor lake health. As mentioned, we believe that our results showed that lakes in Waikato tended to have particularly poor health. What is curious is that to apply this to the real world we googled "Region with the worst Lake Health in New Zealand" and found the article "The Last Lake: Waikato lakes among most polluted in the world - scientist" @Waikato.

If we were to continue researching this data we would like to examine some of the other variables from the original dataset accessed from StatsNZ. We would also like to meet up with the group who examined rivers in New Zealand and discuss the similarities between our findings and datasets.

Our final recommendation is that, to combat the poor health of New Zealand lakes, the resources to tend to this should be focused to the North Island, as the South Island lakes displayed better health, in terms of our Lake Health variables. Specifically, these resources should be focused on the Waikato region, as we found the lakes in this region tended to be of poorer health than other regions. These resources should also be focused to lakes that do not have predominantly native landcover.  
